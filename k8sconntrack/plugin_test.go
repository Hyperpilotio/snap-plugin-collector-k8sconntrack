...
import (
	...
	"github.com/hyperpilotsdi-x/snap/control/plugin"
	"github.com/hyperpilotsdi-x/snap/core"
	"github.com/hyperpilotsdi-x/snap/core/cdata"
	"github.com/hyperpilotsdi-x/snap/core/ctypes"
)
	...
	Convey("Given df plugin is badly initialized", dfp.T(), func() {
		...
		Convey("When list of available metrics is requested", func() {
			mts := []plugin.MetricType{
				plugin.MetricType{
					Namespace_: core.NewNamespace("hyperpilot", "netfilter", "iptables", "rootfs", "space_free"),
					Config_:    node,
				},
			}
	...
	Convey("Given df plugin is initialized", dfp.T(), func() {
		...
		Convey("When values for given metrics are requested", func() {
			...
			Convey("Then proper metrics are returned", func() {
				...
				So(ns, ShouldContain, "/hyperpilot/netfilter/iptables/filter/input")
				So(ns, ShouldContain, "/hyperpilot/netfilter/iptables/filter/output")
				So(ns, ShouldContain, "/hyperpilot/netfilter/iptables/filter/forward")

				So(ns, ShouldContain, "/hyperpilot/netfilter/iptables/nat/prerouting")
				So(ns, ShouldContain, "/hyperpilot/netfilter/iptables/nat/postrouting")
				So(ns, ShouldContain, "/hyperpilot/netfilter/iptables/nat/output")

				So(ns, ShouldContain, "/hyperpilot/netfilter/iptables/mangle/prerouting")
				So(ns, ShouldContain, "/hyperpilot/netfilter/iptables/mangle/output")
				So(ns, ShouldContain, "/hyperpilot/netfilter/iptables/mangle/forward")
				So(ns, ShouldContain, "/hyperpilot/netfilter/iptables/mangle/input")
				So(ns, ShouldContain, "/hyperpilot/netfilter/iptables/mangle/postrouting")

                So(ns, ShouldContain, "/hyperpilot/netfilter/iptables/raw/prerouting")
				So(ns, ShouldContain, "/hyperpilot/netfilter/iptables/mangle/output")


				So(ns, ShouldContain, "/hyperpilot/netfilter/conntrack/transactions")
			})
		})
	})
}
...
func (dfp *DfPluginSuite) TestCollectMetrics() {
	Convey("Given df plugin is initialized", dfp.T(), func() {
		...
		Convey("When list of metrics is requested with too short namespace", func() {
			mts := []plugin.MetricType{
				plugin.MetricType{
					Namespace_: core.NewNamespace("iptables", "rootfs", "space_free"),
				},
			}
			metrics, err := dfPlg.CollectMetrics(mts)
		...
		Convey("When list of metrics is requested with bad namespace", func() {
			mts := []plugin.MetricType{
				plugin.MetricType{
					Namespace_: core.NewNamespace("hyperpilot", "netfilter", "iptables", "rootfs"),
				},
			}
			metrics, err := dfPlg.CollectMetrics(mts)
		...
		Convey("When list of specific metrics is requested", func() {
			...
			mts := []plugin.MetricType{
				...
				plugin.MetricType{
					Namespace_: core.NewNamespace("hyperpilot", "netfilter", "iptables", "big", "inodes_percent_free"),
					Config_:    node,
				},
			}
			metrics, err := dfPlg.CollectMetrics(mts)
		...
		Convey("When one single available dynamic metrics is requested", func() {
			...
			mts := []plugin.MetricType{
				plugin.MetricType{
					Namespace_: core.NewNamespace("hyperpilot", "netfilter", "iptables", "*", "space_free"),
					Config_:    node,
				},
			}
			metrics, err := dfPlg.CollectMetrics(mts)
		...
		Convey("When all available dynamic metrics are requested for all mountpoints", func() {
			...
			mts := []plugin.MetricType{
				plugin.MetricType{
					Namespace_: core.NewNamespace("hyperpilot", "netfilter", "iptables", "*", "*"),
					Config_:    node,
				},
			}
			metrics, err := dfPlg.CollectMetrics(mts)
